...
function linkClassName(active: boolean): string {
  return [
    "rounded px-2 py-1",
    active ? "bg-slate-100 font-medium text-slate-900" : "hover:bg-slate-100",
  ].join(" ");
}
        ...
        return (
          <div key={section.id} className="mt-3 first:mt-1">
            <button
              ...
              className="flex w-full items-center gap-1 rounded px-2 py-1 text-left text-xs font-semibold uppercase tracking-wide text-slate-400 hover:bg-slate-50 hover:text-slate-600"
            >
              <span
                aria-hidden
